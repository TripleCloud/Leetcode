...
给一个数组，按顺序存储的每天的股票价格，只能一次买进卖出，求最大的利润，挺经典的一道题，大概就是2个指针，一个求当前最小值，另一个遍历数组

第一轮  
1）十进制数转七进制数  
2）最大路径和  
3) 打印合法的括号匹配数，比如<<>>合法，<<<>不合法  
问项目  
go垃圾回收机制  
go rehashmap  
KAFKA消息队列和RMQ对比  
...
1）将一个有序数组转为BST  
Zk实现分布式锁  
设计打卡系统，每连续7天打卡送大礼包，怎么存储维护  
问项目  
  
第三轮  
1）求一个数字字符串的下一个更大的，比如1234下一个是1243，如何优化  
TCP如何保证可靠性  
设计微博系统，有关注、粉丝关系，要求看到关注关系、粉丝关系、以及相互关注消息
